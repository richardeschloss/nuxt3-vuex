...
import { defineNuxtModule, addPlugin, addAutoImport, addTemplate } from '@nuxt/kit'
import glob from 'glob'

export const register = {
  vuexStores({ fullStoreDir }) {
...
export default defineNuxtModule({
  setup(moduleOptions = {}, nuxt) {
    ...
    if (existsSync(fullStoreDir)) {
      register.vuexStores({ fullStoreDir })
      addPlugin({
        src: resolve(__dirname, 'plugin.js')
      })
      if (watchStore) {
        nuxt.options.watch.push(fullStoreDir)
      }
    }
  }
})